...
from app.tabs.pvnet_label_manager_tab import PVNetLabelManagerTab
from app.tabs.pvnet_test_tab import PVNetTestTab
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.pvnet_label_manager_tab = PVNetLabelManagerTab()
        self._add_leaf("3. PVNet 라벨 관리", self.pvnet_label_manager_tab)

        self.pvnet_train_tab = PVNetTrainTab()
        self._add_leaf("4. PVNet 학습", self.pvnet_train_tab)

        self.pvnet_test_tab = PVNetTestTab()
        self._add_leaf("5. PVNet 테스트", self.pvnet_test_tab)

        self.cad_model_settings_tab = CADModelSettingsTab()
        self.cad_settings_nav_item = self._add_leaf("6. CAD 모델 설정", self.cad_model_settings_tab)
        ...
        self.pvnet_label_manager_tab.log_message.connect(self.log_console.append_log)
        self.pvnet_train_tab.log_message.connect(self.log_console.append_log)
        self.pvnet_test_tab.log_message.connect(self.log_console.append_log)
        ...
        self.pvnet_test_tab.open_settings_requested.connect(self._on_open_settings)
        ...
        self.pvnet_test_tab.open_cad_settings_requested.connect(self._on_open_cad_settings)
