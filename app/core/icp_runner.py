...
class ICPParams:
    ...
    # 2026-09 추가: 원기둥형(회전대칭) 부품의 "축방향 미끄러짐"(aperture
    # problem) 보정. 원기둥 표면은 대칭축 방향으로는 점 간 거리/법선 잔차가
    # 거의 안 변해서 일반 ICP(point-to-point든 point-to-plane이든)가 그
    # 방향 병진을 사실상 구속하지 못한다 - 볼트 머리 같은 비대칭 특징이
    # 정합에 충분히 반영되지 않으면 축을 따라 임의 위치로 미끄러질 수 있다.
    # "none"(기본, 기존과 완전히 동일)이면 아무 것도 안 한다. "x"/"y"/"z"를
    # 주면 ICP가 다 수렴한 뒤, CAD 로컬 좌표계의 그 축 방향으로만 병진
    # 오프셋을 그리드 서치해서 fitness가 가장 좋은 위치를 추가로 찾는다
    # (refine_axial_translation 참고). 부품의 실제 대칭축은
    # scripts/check_cad_symmetry_axis.py로 미리 확인해둘 것 - manual_labeling_tab의
    # "대칭축" 콤보와 같은 축 표기 관례(CAD 로컬 좌표계 기준)를 쓴다.
    axial_refine_axis: str = "none"
    axial_refine_range_m: float = 0.02     # 탐색 범위 (±m, 부품 길이의 절반 이하로 잡을 것)
    axial_refine_step_m: float = 0.0005    # 탐색 간격 (m)
...
def refine_axial_translation(
    T: np.ndarray, fit: float, rmse: float,
    cad_source: o3d.geometry.PointCloud, scene_source: o3d.geometry.PointCloud,
    params: ICPParams,
) -> tuple[np.ndarray, float, float, list[dict]]:
    """원기둥형 부품의 축방향 aperture problem 보정 (params.axial_refine_axis).

    이미 수렴한 T는 회전은 대체로 맞다는 전제 하에, CAD 로컬 좌표계의
    axial_refine_axis 방향으로만 -range~+range를 step 간격으로 그리드
    서치하면서 evaluate_registration()의 fitness가 가장 좋아지는 오프셋을
    찾는다. run_icp_with_pca_candidates()와 같은 "여러 후보 만들고 fitness로
    채택" 패턴이다 - 다만 여긴 축 하나(1자유도)만 훑으므로 evaluate_registration
    (가벼운 KD-tree 최근접 쿼리, 반복 최적화 없음)만으로 충분하고 비용이 작다.

    params.axial_refine_axis == "none"이면 (T, fit, rmse, [])를 그대로
    반환한다 (하위 호환 - 기존 호출부/기본 동작 완전히 동일).

    주의: 이건 "회전은 맞는데 축 위치만 못 잡는" 경우를 보완하는 것이지,
    회전 자체가 틀린 경우(예: PCA 후보 자체가 다 실제 자세와 다름)는
    못 고친다 - 그런 경우 use_pca_init부터 확인할 것.
    """
    axis = params.axial_refine_axis
    if axis not in _AXIAL_REFINE_AXES:
        return T, fit, rmse, []

    axis_local = _AXIAL_REFINE_AXES[axis]
    ...
    max_dist = params.icp_stages[-1]["max_dist"]
    n_steps = max(1, int(round(params.axial_refine_range_m / max(params.axial_refine_step_m, 1e-6))))

    best_T, best_fit, best_rmse = T, fit, rmse
    for k in range(-n_steps, n_steps + 1):
        if k == 0:
            continue
        delta = k * params.axial_refine_step_m
        T_cand = T.copy()
        T_cand[:3, 3] = T[:3, 3] + delta * axis_world
        res = o3d.pipelines.registration.evaluate_registration(cad_source, scene_source, max_dist, T_cand)
        if (float(res.fitness) > best_fit) or (
            float(res.fitness) == best_fit and float(res.inlier_rmse) < best_rmse
        ):
            best_T, best_fit, best_rmse = T_cand, float(res.fitness), float(res.inlier_rmse)

    log = [{
        "stage": "axial_refine", "axis": axis,
        "range_m": params.axial_refine_range_m, "step_m": params.axial_refine_step_m,
        "n_candidates": 2 * n_steps, "fitness_before": fit, "fitness_after": best_fit,
    }]
    return best_T, best_fit, best_rmse, log
...
def run_icp_for_instance(instance_id: int, pts_mm: np.ndarray, cad_pcd,
    ...
    # 2026-09 추가: 원기둥형 부품 축방향 보정. axial_refine_axis="none"이면
    # refine_axial_translation()이 그대로 통과시키므로 기본 동작은 안 바뀐다.
    cad_for_axial_refine = cad_visible_flipped if flipped else cad_visible_normal
    T, fit, rmse, axial_logs = refine_axial_translation(T, fit, rmse, cad_for_axial_refine, sc, p)
    if axial_logs:
        stage_logs = stage_logs + axial_logs
