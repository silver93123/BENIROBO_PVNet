...
def main() -> int:
    ...
    print(f"[icp_viewer] geometry {len(geometries)}개 로드 완료, draw() 호출 시작 (창이 뜰 때까지 몇 초 걸릴 수 있음)", flush=True)
    o3d.visualization.draw(
        geometries, title=args.title, width=1100, height=780, show_ui=True,
    )
    print("[icp_viewer] draw() 반환됨 (창이 닫혔거나 렌더링을 못 했다는 뜻)", flush=True)
    return 0
