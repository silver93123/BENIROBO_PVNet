...
class SettingsTab(QWidget):
    ...
    def _build_icp_params_box(self) -> QGroupBox:
        ...
        grid.addWidget(QLabel("축방향 보정 축"), 7, 0)
        self.combo_axial_refine_axis = QComboBox()
        self.combo_axial_refine_axis.addItems(["none", "x", "y", "z"])
        idx = self.combo_axial_refine_axis.findText(defaults.axial_refine_axis)
        self.combo_axial_refine_axis.setCurrentIndex(max(0, idx))
        self.combo_axial_refine_axis.setFixedWidth(90)
        grid.addWidget(self.combo_axial_refine_axis, 7, 1)
        self.spin_axial_refine_range = add_double(8, 0, "탐색범위 ±(m)", defaults.axial_refine_range_m, 0.001, 0.2, 0.001, 4)
        self.spin_axial_refine_step = add_double(8, 1, "탐색간격 (m)", defaults.axial_refine_step_m, 0.0001, 0.02, 0.0001, 4)
        axial_hint = QLabel(
            "볼트처럼 원기둥형 부품은 대칭축 방향으로 ICP가 위치를 못 구속해서\n"
            "정합이 축을 따라 미끄러질 수 있습니다. \"none\"이면 기존과 동일(끔).\n"
            "x/y/z를 고르면 ICP 수렴 후 CAD 로컬 좌표계의 그 축 방향으로만\n"
            "그리드 서치해서 fitness가 최대인 위치를 추가로 찾습니다 - 실제\n"
            "대칭축은 scripts/check_cad_symmetry_axis.py로 먼저 확인할 것.\n"
            "회전 자체가 틀린 경우는 못 고치니 use_pca_init을 먼저 켜보세요."
        )
        axial_hint.setStyleSheet("color: #888; font-size: 10px;")
        axial_hint.setWordWrap(True)
        grid.addWidget(axial_hint, 9, 0, 1, 4)
    ...
    def _widgets_to_dict(self) -> dict:
        return {
            ...
            "axial_refine_axis": self.combo_axial_refine_axis.currentText(),
            "axial_refine_range_m": self.spin_axial_refine_range.value(),
            "axial_refine_step_m": self.spin_axial_refine_step.value(),
    ...
    def _apply_dict_to_widgets(self, settings: dict) -> None:
        ...
        idx = self.combo_axial_refine_axis.findText(settings.get("axial_refine_axis", "none"))
        self.combo_axial_refine_axis.setCurrentIndex(max(0, idx))
        self.spin_axial_refine_range.setValue(settings.get("axial_refine_range_m", 0.02))
        self.spin_axial_refine_step.setValue(settings.get("axial_refine_step_m", 0.0005))
