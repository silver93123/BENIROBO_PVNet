...
class ICPWorkbenchTab(Viewer3DMixin, QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self.label_preview_alpha_slider = self._add_opacity_control(
            opacity_row, "라벨 미리보기", DEFAULT_LABEL_PREVIEW_ALPHA, self.image_viewer.set_label_preview_alpha,
        )
        opacity_row.addStretch(1)

        # 2026-09 수정: 슬라이더가 3개일 때는 안 보이던 문제였는데, 4번째
        # ("라벨 미리보기")를 추가하니 이 줄의 최소 너비가 center_widget의
        # setMinimumWidth(320)보다 커져서 전체 창의 최소 너비를 밀어올렸다 -
        # 화면(특히 원격 X11/VNC 환경)에서 더 이상 최대화할 여지가 없다고
        # 판단한 창 관리자가 최대화 버튼을 비활성화하는 증상으로 이어졌다.
        # 이 줄을 가로 스크롤 영역으로 감싸서, 슬라이더가 앞으로 몇 개 더
        # 늘어나도 창 전체 최소 너비에는 영향을 주지 않게 한다 (내용이 넘치면
        # 이 줄 안에서만 가로 스크롤됨). params_scroll과 동일한 패턴.
        opacity_row_widget = QWidget()
        opacity_row_widget.setLayout(opacity_row)
        opacity_scroll = QScrollArea()
        opacity_scroll.setWidgetResizable(True)
        opacity_scroll.setWidget(opacity_row_widget)
        opacity_scroll.setFrameShape(QFrame.Shape.NoFrame)
        opacity_scroll.setFixedHeight(44)
        opacity_scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAsNeeded)
        opacity_scroll.setVerticalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        center.addWidget(opacity_scroll)
    ...
    def _add_opacity_control(layout, label: str, default_alpha: int, setter) -> "QSlider":
        """오버레이 요소 하나(마스크/라벨박스/CAD 오버레이/라벨 미리보기)의
        투명도 슬라이더를
    ...
    def _build_pose_edit_row(self, instance_id: int) -> QWidget:
        ...
        # Pitch가 ±90°에 가까울 때 Roll/Yaw가 서로 얽히는 짐벌락 때문에,
        # 칸 하나만 바꿔도 전혀 다른 자세로 튀어버리는 문제가 있었다
        # (build_icp_init 등과 같은 Rz@Ry@Rx 합성 관례 자체는 맞지만, 절대값
        # 3개를 한꺼번에 재조합하는 편집 방식이 문제였다). 이제는 "바뀐 칸
        # 하나"의 변화량(delta)만 카메라 좌표계 그 축으로 현재 회전행렬
        # 위에 곱해서 증분 적용한다 - 항상 "지금 자세에서 그 축으로만 delta
        # 만큼 더 돈다"는 의도대로 동작하고, 다른 두 칸의 현재값과 무관하다.
        # last_euler는 "마지막으로 스핀박스에 반영된 값"을 기억해서 다음
        # 변경분(delta)을 계산하기 위한 것 - 실제 회전 적용에는 안 쓰인다
        # (실제 적용은 항상 target.T의 현재 회전행렬 기준 증분).
        last_euler = {
            "roll": euler0["roll_deg"], "pitch": euler0["pitch_deg"], "yaw": euler0["yaw_deg"],
        }

        def on_pos_changed(_value=None):
            self._apply_position_edit(
                instance_id, pos_spins["x"].value(), pos_spins["y"].value(), pos_spins["z"].value(),
            )

        def make_on_rot_changed(axis_key: str):
            def handler(_value=None):
                new_val = rot_spins[axis_key].value()
                delta_deg = new_val - last_euler[axis_key]
                last_euler[axis_key] = new_val
                if abs(delta_deg) < 1e-9:
                    return
                self._apply_incremental_rotation(
                    instance_id, axis_key, delta_deg,
                    pos_spins["x"].value(), pos_spins["y"].value(), pos_spins["z"].value(),
                )
                self._refresh_rotation_spins(instance_id, rot_spins, last_euler, skip_key=axis_key)
            return handler

        for spin in pos_spins.values():
            spin.valueChanged.connect(on_pos_changed)
        for axis_key, spin in rot_spins.items():
            spin.valueChanged.connect(make_on_rot_changed(axis_key))
        ...
        def on_reset():
            ...
            if target is not None:
                target.T = T_original.copy()
                target.pose = icp_runner.transform_to_pose(target.T)
                target.pick_point_mm = orig_pick_mm
            ...
            last_euler["roll"] = orig_euler["roll_deg"]
            last_euler["pitch"] = orig_euler["pitch_deg"]
            last_euler["yaw"] = orig_euler["yaw_deg"]
    ...
    def _refresh_rotation_spins(
        self, instance_id: int, rot_spins: dict, last_euler: dict, skip_key: str,
    ) -> None:
        """증분 회전 적용 후, 방금 편집한 칸(skip_key)을 제외한 나머지 두 칸의
        표시값을 새 회전행렬 기준으로 다시 추출해 갱신한다.

        방금 편집한 칸은 사용자가 입력한 값 그대로 유지한다 - 재추출값이
        (짐벌락 근방 등에서) 살짝 다르게 나올 수 있는데, 그걸로 사용자 입력을
        덮어쓰면 "숫자를 90으로 쳤는데 표시가 89.7로 바뀌는" 혼란스러운
        경험이 된다. 나머지 두 칸은 실제 회전행렬을 정직하게 반영해야
        하므로 갱신한다."""
        target = next((r for r in self._last_icp_results if r.instance_id == instance_id), None)
        if target is None or target.pose is None:
            return
        euler = target.pose["euler_deg"]
        for key in ("roll", "pitch", "yaw"):
            if key == skip_key:
                continue
            rot_spins[key].blockSignals(True)
            rot_spins[key].setValue(euler[f"{key}_deg"])
            rot_spins[key].blockSignals(False)
            last_euler[key] = euler[f"{key}_deg"]

    def _apply_position_edit(self, instance_id: int, x_mm: float, y_mm: float, z_mm: float) -> None:
        """위치(pick point)만 바꾼다 - 현재 회전행렬은 그대로 유지."""
        target = next((r for r in self._last_icp_results if r.instance_id == instance_id), None)
        if target is None or target.T is None:
            return

        R_current = target.T[:3, :3]
        pick_point_m = np.array([x_mm, y_mm, z_mm]) / 1000.0
        if self._cad_pcd is not None:
            cad_center_m = np.asarray(self._cad_pcd.get_center())
            t_new = pick_point_m - R_current @ cad_center_m
        else:
            t_new = pick_point_m
    ...
    def _apply_incremental_rotation(
        self, instance_id: int, axis_key: str, delta_deg: float,
        x_mm: float, y_mm: float, z_mm: float,
    ) -> None:
        """스핀박스 하나(axis_key)의 변화량(delta_deg)만큼, 카메라 좌표계의
        그 축(고정: X=roll, Y=pitch, Z=yaw) 기준으로 "현재 회전행렬 위에"
        곱해서 증분 회전을 적용한다.

        절대 오일러 3개를 매번 처음부터 재조합(Rz@Ry@Rx)하던 예전 방식은
        Pitch가 ±90°에 가까우면 Roll/Yaw가 서로 얽리는 짐벌락 때문에 칸
        하나만 바꿔도 결과가 완전히 다른 자세로 튀는 문제가 있었다. 여기서는
        "바뀐 축 하나"만 다루므로 다른 두 칸의 현재값이 몇 도든 상관없이
        항상 "지금 보이는 자세에서 그 카메라 축으로 delta_deg만큼 더 돈다"는
        의도대로 동작한다.

        회전은 pick point를 중심으로 일어난다(위치 칸은 그대로 유지) -
        기존 _apply_pose_edit()와 동일한 관례."""
        ...
        delta_rot_fn = {"roll": _Rx, "pitch": _Ry, "yaw": _Rz}[axis_key]
        R_current = target.T[:3, :3]
        # 카메라(월드) 좌표계 기준 증분 회전이므로 왼쪽에서 곱한다(외재적 회전) -
        # "카메라 시점 기준" 기즈모(Roll=카메라X, Pitch=카메라Y, Yaw=카메라Z)와
        # 일치시키기 위함. 물체 로컬축 기준으로 돌리고 싶으면 오른쪽 곱으로
        # 바꾸면 된다.
        R_new = delta_rot_fn(delta_deg) @ R_current
