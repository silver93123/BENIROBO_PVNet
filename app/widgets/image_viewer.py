...
#   센트로이드 cv2 BGR(0,255,255) -> RGB(255,255,0)  (노랑)
# 나머지 키포인트만 저장 파일의 하늘색(cv2 BGR(255,200,0))을 그대로 안 쓰고
# 주황(255,140,0)으로 바꿨다 - 하늘색이 POSE_OVERLAY_BASE_COLOR(CAD
# 오버레이, 역시 하늘색 계열)와 거의 구분이 안 돼서 두 오버레이를 같이 켜면
# 뭐가 뭔지 안 보였기 때문. 저장되는 jpg 파일 자체의 색은 안 바꿨다(이건
# 화면 표시 전용 상수).
DEFAULT_LABEL_PREVIEW_ALPHA = 130  # 0~255
LABEL_PREVIEW_MASK_COLOR = (60, 220, 60)
LABEL_PREVIEW_CENTROID_COLOR = (255, 255, 0)
LABEL_PREVIEW_KEYPOINT_COLOR = (255, 140, 0)  # 주황 - CAD 오버레이(하늘색)와 구분되게
LABEL_PREVIEW_CENTROID_RADIUS = 6.0  # px, 저장된 미리보기 이미지의 cv2.circle(r=6)과 동일
LABEL_PREVIEW_KEYPOINT_RADIUS = 4.0  # px, 저장된 미리보기 이미지의 cv2.circle(r=4)과 동일
...
class ImageViewer(QLabel):
    ...
    def __init__(self, parent=None):
        ...
        self._label_preview_overlays: dict[int, dict] = {}  # obj index -> {"mask", "keypoints_2d"}
        ...
        self._label_preview_alpha = DEFAULT_LABEL_PREVIEW_ALPHA
    ...
    def set_label_preview_alpha(self, alpha: int) -> None:
        """PVNet 라벨 미리보기(마스크 채우기 + 키포인트 채워진 원) 반투명도."""
        self._label_preview_alpha = alpha
        self._refresh()
    ...
    def set_label_preview_overlay(
        self, obj_index: int, mask: np.ndarray | None, keypoints_2d: np.ndarray | None,
    ) -> None:
        """obj_index 인스턴스의 PVNet 라벨 미리보기(마스크 + 키포인트)를
        현재 프레임 위에 얹는다. mask가 None이면 그 인스턴스의 오버레이를
        ...
        """
        if mask is None:
            self._label_preview_overlays.pop(obj_index, None)
        else:
            self._label_preview_overlays[obj_index] = {
                "mask": mask,
                "keypoints_2d": None if keypoints_2d is None else np.asarray(keypoints_2d),
            }
        self._refresh()

    def clear_label_preview_overlays(self) -> None:
        self._label_preview_overlays = {}
        self._refresh()
    ...
    def _refresh(self) -> None:
        ...
        for overlay in self._label_preview_overlays.values():
            ...
            if mask is None:
                continue
            mask_image = self._mask_to_qimage(mask, QColor(*LABEL_PREVIEW_MASK_COLOR), self._label_preview_alpha)
            if mask_image is not None:
                painter.drawImage(0, 0, mask_image)
        ...
        keypoint_color = QColor(*LABEL_PREVIEW_KEYPOINT_COLOR)
        keypoint_color.setAlpha(self._label_preview_alpha)
        for overlay in self._label_preview_overlays.values():
            kpts = overlay.get("keypoints_2d")
            if kpts is None:
                continue
            for idx, (x, y) in enumerate(kpts):
                is_centroid = idx == 0
                painter.setBrush(centroid_color if is_centroid else keypoint_color)
                r = LABEL_PREVIEW_CENTROID_RADIUS if is_centroid else LABEL_PREVIEW_KEYPOINT_RADIUS
